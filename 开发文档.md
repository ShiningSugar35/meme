...
| `backend/app/ml/` | 无泄漏特征、候选 feature pool、chronological split、扩展候选池、`6TP-FP`/E-G-S、单一决策线、奥卡姆子集和 Top 3 工件。 |
...
2026-08-12 正式 run `89c2d088-ea1b-4bc7-9aec-2ca7a92220b2` 在 2374 mature / 414 positive 上得到 Top 3：RF（12 特征，threshold=0.1420，E=0.2886，G=0.4896，S=0.3690）、DecisionTree（12 特征，0.1100，E=0.3030，G=0.4404，S=0.3580）、GradientBoosting（12 特征，0.1182，E=0.2823，G=0.4697，S=0.3573）。最终 20% certification holdout 的固定 $50 理论收益：RF $875、DT $890、GB $1000；`rules_only` $915。HGB/XGBoost 等在 final 上可有更高局部成绩也不会因此反向改变开发期 Top 3，这是防止测试集过拟合的硬约束。
...
- BUY/SELL 均经过 seeded quote model，包含流动性占比价格影响、随机滑点、1% 平台费、网络费、延迟和可注入失败；网络费原始事实保存为 SOL，并使用 `requested_at + latency` 对应的手续费实际发生时 SOL/USD 折算为 USD 直接扣现金/进入净 PnL；失败不会伪造仓位/平仓；
...
本地 quote 模型不是 GMGN 真实报价，因此四策略模拟 PnL 只代表受控执行模型，不冒充链上真实收益；真实钱包资金同步仍留到实盘阶段。
...
- Top 3 ML：扩展候选池、optional candidate skipped、单一决策线、`6TP-FP`/E-G-S、12/20/全量 one-standard-error 奥卡姆、final holdout 隔离；
- 发布/回滚：Top 3 slot 原子替换、Rank 1/2/3 唯一、三 artifact 可加载、final 不回写排名、历史 retired Rank 1 恢复；
- 训练生命周期：feature request 持久化、周日 03:00 计划点唯一、有限 retry、TrainingWorker 串行消费、进程中断恢复、三 active model 7 日 fixed-payoff capture 退化监控；
- 模拟：四策略同 session、独立账本事务、费用/滑点/延迟/失败、每策略 10 仓、同币跨策略/多批次、1m first-touch、closing 失败重试/重启恢复、session reset/history 隔离；
...
4. 扩展候选池 + 时序 gap + 每模型单一决策线 + `6TP-FP`/E/G/S + 12/20/全量 one-standard-error 奥卡姆选择 + Top 3 独立工件；
...
8. 当前 simulation session 四策略 → Top 3 各自 prediction + `rules_only` 无预测开仓 → 1m first-touch → closing retry/restart → 四策略 PnL/模型审计/session history 的完整链；collector 关闭时仍可 monitor-only 退出；
9. Agent 只读上下文、持久化白名单 proposal、人工 approve/reject 和非实盘执行；live/wallet/secret 类 proposal 在创建阶段直接拒绝；
10. Dashboard/Models/Signals/Portfolio/Runtime/Agent 六页控制台；Top 3/E/G/S/四策略模型审计已展示；后端 98/98 pytest 与前端 production build 通过。
...
- 模拟退出触发前刷新当前 liquidity；token decimals 可审计时，用 Jupiter Token→USDC `GET /swap/v1/quote` 做只读路由验证。明确无路由进入 no-route 重试/终态全损；quote unavailable 不等同 rug，降级到当前 liquidity + 本地冲击模型。整个模拟 route probe 不构建、不签名、不提交交易。
