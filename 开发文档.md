...
| `backend/app/services/collector_worker.py` | 12 Key 角色装配；完整采集或 monitor-only 模式；模拟退出/标签/发现故障隔离。 |
...
| GET | `/api/portfolio` | 默认返回当前 simulation session + live；可显式包含旧模拟 session。 |
| GET | `/api/simulation` | 当前模拟 session 与三账户账本。 |
| POST | `/api/simulation/reset` | 无模拟持仓时关闭旧 session 并创建新的 `1000 USD + 0.1 SOL` 三账户会话。 |
| GET | `/api/simulation/history` | 模拟会话 registry、账户仓位与已实现 PnL 历史。 |
| GET | `/api/runtime` | Runtime、TrainingWorker、model health 与风险门禁。 |
...
五候选均使用固定随机种子。XGBoost 不可用时会记录 skipped，不应导致其余四模型静默失败。候选必须同时通过开发期和最终留出的 20% Precision 门槛与最低交易数。
...
- `/signals`：概率/阈值/入选/成熟标签/模型版本；
- `/portfolio`：当前 simulation session 的 paper/shadow/live 持仓、平仓记录、simulation session 历史与清仓/重置控制；
