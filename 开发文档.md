...
| `backend/app/ml/` | 无泄漏特征、chronological split、扩展候选池、`3TP-FP`/E-G-S、fold-train-only 自适应特征维度、单一决策线、`E_exec` shadow 指标和 Top 3 工件。 |
...
**自 2026-08-22 起当前生产口径**：Top10 准入改为严格 `0.14 < top_10_holder_rate < 0.25`，边界 `0.14/0.25` 均不接受。准入判断与模型特征统一读取 `normalize_token()` 后的同一权威 `top_10_holder_rate`；discovery prefilter、authoritative `SafetyFilter`、CsvImporter 与 legacy 采集脚本均执行相同严格条件。迁移前备份 `data/backups/meme_quant_pre_top10_014_025_20260822T145311Z.db`；从 1009 条样本删除 210 条不合规样本（200 mature：28 正 / 172 负；10 pending），158 个相关 rules-only simulation positions 只解除 sample 引用以保留账本/成交，其中 2 个当时 open 的仓位继续自然退出；迁移后 799 条样本（789 mature：157 正 / 632 负；10 pending），新规则违规数与外键违规数均为 0。
...
上面的逐笔本金/return 只用于真实模拟执行回放；旧数据缺 raw liquidity 时不能伪造成逐笔真实 USD PnL。模型排名另用统一固定 $50 的理论收益：`profit_units = 3×TP - FP`，`fixed_profit_usd = 5×profit_units`。这允许 legacy binary tag 与新样本在同一分类经济尺度上比较，同时不改变真实资金曲线。

每个候选模型只搜索一个开发期 OOS 决策线。经济得分 `E` 使用各开发 fold 的 `clip((3TP-FP)/(3N+),-1,1)` 平均；泛化得分 `G` 综合 Average Precision 相对基准率的 AP Skill、fold 稳定性和时间衰减；综合分固定 `S=0.60E+0.40G`。Precision/Recall 作为审计指标保留，但不再以三档硬门槛驱动模型。
...
- Top 3 ML：扩展候选池、optional candidate skipped、单一决策线、`3TP-FP`/E-G-S、自适应 4..N 特征数、fold-train-only TreeSHAP interaction 排序、one-standard-error + 8% 相对 S 损失双门槛奥卡姆、8% 性能带内 model-family 多样性、final holdout correlation/Jaccard 只读审计与隔离、`E_exec` 影子指标权重恒为 0；
...
4. 扩展候选池 + 时序 gap + 每模型单一决策线 + `3TP-FP`/E/G/S+ 自适应 4..N / fold-train-only TreeSHAP interaction 特征排序 + one-standard-error 与 8% 相对 S 损失双门槛奥卡姆 + 8% 性能带内 model-family 多样性 Top 3 + final holdout correlation/Jaccard 审计；`E_exec` 只做权重为 0 的 shadow 指标；
...
- 2026-08-12 当时 `E = mean(clip((6TP-FP)/(6N+),-1,1))`；2026-08-14 当时改为 `E = mean(clip((5TP-FP)/(5N+),-1,1))`；2026-08-18 当时进一步收紧为 `E = mean(clip((4TP-FP)/(4N+),-1,1))`；**2026-08-24 当前进一步收紧为 `E = mean(clip((3TP-FP)/(3N+),-1,1))`**。`G` 与 `S=0.60E+0.40G` 结构保持不变；`E_exec` 仍仅作为 shadow/audit 指标，当前排名权重固定为 0。
...
- 2026-08-18 当时经济代理收紧为 `+4/-1`：`U=4TP-FP=N+×recall×(5-1/precision)`，`E=mean(clip((4TP-FP)/(4N+),-1,1))`，固定 $50 下正类代理 +$20、负类 -$5，盈亏平衡 Precision=20%，模型版本为 `fixed_4_to_1_v2`。
- **2026-08-24 当前经济代理为 `+3/-1`**：`U=3TP-FP=N+×recall×(4-1/precision)`，`E=mean(clip((3TP-FP)/(3N+),-1,1))`，固定 $50 下正类代理 +$15、负类 -$5，盈亏平衡 Precision=25%。`ECONOMIC_OBJECTIVE_VERSION=fixed_3_to_1_v3` 写入新模型；Prediction 对旧 objective active Top3 fail-closed。ModelHealth 与 Adaptive Policy 统一读取 `WIN_UNITS=3`，禁止口径分裂。
- 热更新手动重训 run `b040425e-3043-4dd5-9752-f6685d83f4b9` 使用 1080 条 current-generation mature 样本和持久化 44 特征候选池，`retry_count=0`。开发期新 Top 3 为 Extra Trees（35 features，threshold `0.2454529`，E/G/S=`0.139676/0.459697/0.267685`）/ Decision Tree（4 features，threshold `0.42`，E/G/S=`0.124905/0.437260/0.249847`）/ Random Forest（20 features，threshold `0.2763549`，E/G/S=`0.118388/0.447422/0.250002`），三者 objective 均为 `fixed_3_to_1_v3`。训练完成后进入 `waiting_for_flat` 并冻结四策略新开仓，已有 rules-only 仓按原退出规则自然结束后由 TrainingWorker 原子激活。
...
- Adaptive Policy 使用三个动作 `DEFENSIVE / NEUTRAL / EXPANSIVE`，分别对每个模型自己的 base threshold 施加有限 logit offset；`rules_only` 永不调用 adaptive。三个模型每次仍记录 neutral counterfactual。V2 默认强制 shadow-only：Regime 即使推荐非 Neutral，实际 action 仍为 Neutral，直到同一 feature generation 至少 120 个独立 sample cluster、至少 40 个 policy-separating cluster，chronological 70/30 development/certification 的 +3/-1 shadow excess utility 满足 development mean>0、certification 95% LCB>0 且三个模型 certification mean 均>=0。5% exploration 另需 `adaptive_exploration_ready=true` 第二道持久化证据门，因此仅改环境变量不能提前在线探索。
...
### 2026-08-19 Windows Modern Standby 24h 采集修订

- 现场审计 HP ProBook 450 G8 / Windows 10 19045：该机只支持 `S0 Low Power Idle (Connected)`，不支持传统 S3。交流电的 `STANDBYIDLE=0`、`HIBERNATEIDLE=0` 均已是“永不”，但显示器 `VIDEOIDLE=180s`；系统日志仍记录 Kernel-Power 506 `Idle Timeout` 进入连接待机，桌面 Python/uvicorn 在该状态被暂停。
- 2026-08-18 22:07:30 BJT 为最后一个 Collector cycle，下一 cycle 直接到 2026-08-19 08:06:11 BJT；runtime supervisor 自 8 月 16 日持续存活、Windows 自 8 月 8 日未重启，排除进程崩溃/重启。Kernel-Power 同时记录 22:21 进入连接待机、23:23 键盘短唤醒后再次待机、08:07 键盘唤醒，时间线与采集空窗一致。
- 新增 `SystemAwakeService`：默认 `PREVENT_SLEEP_WHILE_COLLECTING=true`。仅在 Windows、后台 worker 与 Collector 启用且 `GetSystemPowerStatus` 确认接交流电时，调用 `SetThreadExecutionState(ES_CONTINUOUS|ES_SYSTEM_REQUIRED)`；显示器仍可正常熄灭，但 idle-triggered Modern Standby 被抑制。拔电、禁用 Collector 或后端退出时调用 `ES_CONTINUOUS` 释放请求，避免电池场景长期强制唤醒。
- 运行态写入 `system_awake_request`；当前现场已验证 `state=held_on_ac / ac_line_status=1 / system_required=true`，后端健康检查与 Collector 均正常。


### 2026-08-25 Phase16 v5 / 90m / drift-safe 生产收口

- 标签与退出合同正式切换为 `sl090_tp180_m90_binary_v5`：SL=0.9、TP=1.8、window=5400s。`price_1h_*` 作为 legacy 事实保留，新标签只写 generic barrier audit；新仓写入 `m90_tp180_sl090_v2 + 0.9/1.8/5400s` 不可变快照，旧仓继续按 `h1_tp160_sl090_v1 + 0.9/1.6/3600s` 解释。生产迁移当时 1216 mature 全量完成，186 positive，Kline 1216/1216，label/tag/generic-barrier/FK violation=0；物理备份位于 `data/backups/`，不入 Git。
- 2026-08-24 Collector/PositionMonitor 中断事故中两笔延迟退出污染仓被标记 `performance_excluded=true`。异常仓原始净收益 `+1716.125107... USD`、Jupiter quote、fill、trigger/settlement time 均保留，不做“改价”；但 Dashboard、simulation cash reconciliation、胜负/realized PnL、execution evidence 均忽略这两笔污染事实。
- 昨夜约 15h 采集空窗的系统级根因锁定为 Windows Modern Standby。旧 `SetThreadExecutionState` 存在线程生命周期导致的假 held；`SystemAwakeService` 改为持久 `PowerCreateRequest/PowerSetRequest`，交流电时同时持有 `SystemRequired + ExecutionRequired`，运行态写 `mechanism=power_request_system_and_execution_required`。非管理员进程无法调用 `powercfg /requests`，但 Power Request API 返回成功、runtime state 与持续采集共同作为无 UAC 验收证据。
- Phase16 新增 chronological sigmoid/Platt calibration、model-specific sparse budget、age gate、execution-risk head、development-reference PSI drift、final deployment certification。RBF-SVM 不再使用 `SVC(probability=True)` 内部随机校准；final holdout 只做 `.transform + certification`，绝不参与 feature/threshold/budget/calibrator fit。
- durable manual run `e83855f9-158d-4061-ab80-ba7aaa49c2a6` 使用 current v5 mature + 44 项批准候选池完成，`retry_count=0`。候选 Top3 为 Gradient Boosting / CatBoost / LightGBM，production artifacts 均可加载且 provenance 一致；execution-risk head 722 observations、ExtraTrees holdout AUC≈0.6933、AP≈0.4279，认证通过。
- 本代 **deployment certification 被安全拒绝**：final window=244 rows/27 positives，三模型完整 model-policy selected=0；development positive prior≈16.34%，final≈11.07%，`ln(liquidity_usd)` PSI≈0.4617 达 severe。对原始流动性分位复核显示 final 期低端由约 `$4.8k` 收缩到 `$7.6k+`，后续新样本中位继续上移到约 `$8.9k`，属真实分布漂移而非 PSI 代码错误。禁止通过降低 0.25 global floor、绕过 age/risk/drift gate 或用 final holdout 反向调参来“制造通过”。
- 因认证失败，旧 2026-08-23 active Top3 仍保留在 slot 作为历史对象，但 Prediction 因 label/decision-policy provenance stale 返回 `active_top3_phase16_contract_stale`，模型策略不再开仓。候选本代 `activation.status=blocked_certification`，未冒充 active。
- 新增 recovery fallback：Scheduler 对 `insufficient_data / active_top3_not_ready / active_top3_phase16_contract_stale / deployment_certification_blocked` 自动切到 daily 17:00 BJT；一旦合规 Phase16 generation 激活且 health 恢复，则自动回 weekly。该策略允许真实漂移数据继续积累后再训练，不要求人工持续盯守。
- 运行收口时发现 `.env` 的 `SIMULATION_ENABLED=false` 使 `rules_only` 静默短路，这是模型之外的第二个生产可用性问题。已改回 `SIMULATION_ENABLED=true` 并重启 runtime；`DRY_RUN=true` 明确保留。随后实测 `rules_only` 已开出新 `m90_tp180_sl090_v2` 仓位，证明 Collector→sample→Prediction baseline→paper entry→PositionMonitor 链路恢复。
- 最终后端全量回归 **191/191 pass**；scheduler recovery targeted 12/12；frontend `tsc -b && vite build` pass。主 Phase16 实现已提交并 push：`26db419 feat: productionize phase16 sparse risk policy`。
