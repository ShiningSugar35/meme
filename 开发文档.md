...
- Top 3 ML：扩展候选池、optional candidate skipped、单一决策线、`6TP-FP`/E-G-S、自适应 4..N 特征数、fold-train-only TreeSHAP interaction 排序、one-standard-error + 8% 相对 S 损失双门槛奥卡姆、8% 性能带内 model-family 多样性、final holdout correlation/Jaccard 只读审计与隔离、`E_exec` 影子指标权重恒为 0；
...
4. 扩展候选池 + 时序 gap + 每模型单一决策线 + `6TP-FP`/E/G/S（评分公式保持不变）+ 自适应 4..N / fold-train-only TreeSHAP interaction 特征排序 + one-standard-error 与 8% 相对 S 损失双门槛奥卡姆 + 8% 性能带内 model-family 多样性 Top 3 + final holdout correlation/Jaccard 审计；`E_exec` 只做权重为 0 的 shadow 指标；
