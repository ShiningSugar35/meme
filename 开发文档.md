...
| `backend/app/services/collector_worker.py` | 12 Key 角色装配；完整采集或 monitor-only 模式；模拟退出/标签/发现故障隔离；双生命周期独立统计与最近 250 条结构化实时事件缓冲。 |
| `backend/app/ml/` | 无泄漏特征、chronological split、扩展候选池、`6TP-FP`/E-G-S、fold-train-only 自适应特征维度、单一决策线、`E_exec` shadow 指标和 Top 3 工件。 |
...
| `backend/app/services/paper_position_monitor.py` | 1m first-touch、当前 liquidity/risk 快照、Jupiter 只读卖出 route、quoted/no-route/unavailable 分流与 closing 重启续跑。 |
...
| `training_runs` | request JSON、`scheduled_for`、retry、model/promoted/summary | durable manual/daily/weekly/startup queue；`promoted=0 + summary.activation=waiting_for_flat` 表示候选已训练、等待三模型空仓，跨重启继续有效。 |
...
| GET | `/api/models` | `active_models` Top 3、历史模型版本、候选 feature catalog/coverage 与最近 durable training runs。 |
...
候选池固定随机种子并覆盖 LR、DecisionTree、HGB、GradientBoosting、AdaBoost、ExtraTrees、RandomForest、RBF-SVM、XGBoost；LightGBM/CatBoost/FLAML 为可选依赖，缺失时明确 `skipped`。FLAML/AutoML 若启用，只能在 outer train 内部使用时间切分，不能看到最终 holdout。Top 3 排名只使用开发期 chronological OOS；最终留出不设反向晋级门槛，只做 certification。
...
上面的逐笔本金/return 只用于真实模拟执行回放；旧数据缺 raw liquidity 时不能伪造成逐笔真实 USD PnL。模型排名另用统一固定 $50 的理论收益：`profit_units = 6×TP - FP`，`fixed_profit_usd = 5×profit_units`。这允许 legacy binary tag 与新样本在同一分类经济尺度上比较，同时不改变真实资金曲线。

每个候选模型只搜索一个开发期 OOS 决策线。经济得分 `E` 使用各开发 fold 的 `clip((6TP-FP)/(6N+),-1,1)` 平均；泛化得分 `G` 综合 Average Precision 相对基准率的 AP Skill、fold 稳定性和时间衰减；综合分固定 `S=0.60E+0.40G`。Precision/Recall 作为审计指标保留，但不再以三档硬门槛驱动模型。

每个算法不再固定比较 12 / 20 / 全量档位，而是在可行特征数上自适应搜索。每个 chronological development fold 的特征排序只能使用该 fold 训练段，OOS 测试段不得参与特征选择；再用 one-standard-error 规则选择仍与算法最佳分统计上近似等价的最小维度。随后只按开发期 `S` 选 Top 3；最终 holdout 在选择完成后才附加到审计指标，绝不进入 Top 3 排序。Top 3 各自以选定特征和单一决策线生成独立 bundle。

### 8.4 Top 3 原子发布与真实研究结果

`TrainingService` 在一次 durable run 内先注册三个候选模型工件，但**训练完成不再等于立即发布**。run 先以 `completed + promoted=0 + activation.waiting_for_flat` 持久化；`TrainingWorker` 每个轮询周期检查 `model_1/2/3` 是否仍有 `opening/open/closing/manual_intervention` 仓位。只有三模型全部空仓时，才通过 `active_model_slots` 同事务替换当前 Top 3，并重置三个模型策略的 $1000 模拟账本与上线期统计。发布前必须满足：
...
2026-08-12 H1/no-completed 迁移后正式 run `ca922c9b-a835-486b-ba61-e94c04978399` 使用 2292 条当时成熟 H1 样本，激活 Extra Trees / AdaBoost / Random Forest。该 generation 是本轮自适应特征选择上线前生成的冻结工件，继续按其原特征/threshold 运行；新的 4..N、fold-train-only 特征选择从后续训练 run 生效，不追溯改写现役模型。无论新旧流程，最终 holdout 都只做 certification，绝不反向改变开发期 Top 3。

### 8.5 调度和退化监控

自动调度统一使用 `Asia/Shanghai`：正常状态按 `training_weekday`（当前周日）17:00 训练；`ModelHealthService` 为 `insufficient_data` 时自动切换为每天 17:00。每个到期点前 1 小时（16:00）先将 `model_entries_paused_for_rollover=true`，只阻止 `model_1/2/3` 新开仓，已有仓位和 `rules_only` 继续运行。17:00 到点直接创建 durable run，不等待持仓清零；关机错过时启动后按唯一 `scheduled_for` 创建 `startup_catchup`。计划失败只重试同一 run，不能通过创建新计划绕过冻结。

7 日 `ModelHealthService` 会分别统计三个 active model 的成熟 OOS prediction，并按近期 `6TP-FP` economic capture 与各模型训练时 final capture 比较，但从 v10 起只负责状态分类：`insufficient_data` 驱动每日 cadence，其他状态（含 `healthy/degraded`）使用周 cadence，不再直接创建绕过 16:00/17:00 生命周期的插队训练。候选训练完成后可跨重启等待三模型空仓；空仓后 `TrainingWorker` 立即换代并解除 entry gate。历史可加载的 retired Rank 1 支持人工恢复到 slot 1，但回滚同样要求三模型先空仓，并重置三个模型策略的 generation 统计。
...
- 每个 simulation session 创建 `model_1`、`model_2`、`model_3`、`rules_only` 四个独立策略账本，各初始化 `1000 USD`；schema v11 继续保持单一 USD 会计，并新增 model generation 边界：`active_model_slots.selected_at` 是当前模型上线统计起点。自动/人工换模只有在三模型策略全部空仓后发生，届时 `model_1/2/3` 各自重新初始化为 `1000 USD`，旧 generation 的 position/trade/PnL 保留审计但不进入新卡片统计；`rules_only` 是连续无模型基线；
- 单笔 `min(1% liquidity, $50)`，每账户最大 10 仓，同 Token 多批次独立；
- BUY 仍经过 seeded quote model；SELL 触发前先刷新当前 GMGN liquidity，并在 token decimals 可审计时调用 Jupiter Token→USDC `GET /swap/v2/order` 做 quote-only route probe（不传 `taker`）。真实 quote 的 USDC `outAmount` 直接决定模拟卖出 fill；明确无路由进入 `no_route` 重试/终态全损；Jupiter 429、网络或 API unavailable 只降级到“当前 liquidity + seeded 冲击模型”，不得伪装为 rug。全链路继续记录 1% 平台费、网络费、延迟/失败及手续费发生时 SOL/USD 折算；
- `PaperPositionMonitor` 使用 GMGN 1m K 线判断 first-touch：0.9x 止损、1.6x 止盈、1h 到期，同 bar 止损优先；同 Token 多仓合并一次 K 线请求；
...
- 持仓卡片的 generation 统计固定为 8 项：当前余额、持仓本金、已实现收益、累计手续费、持仓数、交易数、Precision、Recall。当前余额=`available cash USD`，BUY 时本金、平台费、费时网络费已经从现金扣除，因此绝不含当前持仓本金；已实现收益=`closed positions.net_pnl_usd`，正常 v9+ 路径逐笔满足 `net_pnl = gross_pnl - entry/exit platform fee - entry/exit/failed-exit network fee`，滑点已进入 fill price，不重复扣。交易数=`当前 model_id + selected_at 之后 status='closed' 的完整仓位数`，因此终态卖出失败也计 1；Precision=`TP/selected`，Recall=`TP/positive_count`，要求 `model_id`、`predicted_at>=selected_at`、`sample.entry_time>=selected_at` 且样本已 mature/tag∈{0,1}，避免上线前 backlog 污染；`rules_only` 对 session 内所有规则准入样本等价预测为正，因此其 Precision=成熟样本正类率、存在正类时 Recall=1.0；
- 模拟盘不维护 SOL reserve，不存在 `paper_sol_reserve_depleted`；`SolUsdPriceService` 将 SOL/USD 分钟级事实冻结到 `asset_usd_prices`。手续费发生时若没有足够新鲜的历史价格，返回 `sol_usd_price_unavailable` 并阻塞该次模拟执行，禁止用当前/日终/平仓后的价格补算；
- `COLLECTOR_ENABLED=false` 但 `PAPER_MARKET_MONITOR_ENABLED=true` 时运行 monitor-only worker，已有模拟仓位退出和 T+1h label finalization 不依赖 discovery 是否开启；Collector/monitor 周期仍会刷新模拟手续费所需的 SOL/USD 事实。
...
默认 `DRY_RUN=true`。当前只保留实盘接口，不实现自动 live BUY；未来实盘使用当时 active Top 3 的 Rank 1 模型及其单一决策线，不再存在 `profile`。实盘准备接口会检查：
...
- `/portfolio`：持仓主工作台。页面先在 `simulation/live` 两种模式间切换；simulation 下再选择 `model_1 / model_2 / model_3 / rules_only`。桌面端四策略卡采用两列布局，每卡 2×4 显示当前余额、持仓本金、已实现收益、累计手续费、持仓数、交易数、Precision、Recall；模型换代时 `model_1/2/3` 全部 generation 指标自动归零，`rules_only` 保持连续无模型基线。当前策略额外展示平台费、网络费 USD + 原始 SOL 数量、滑点损耗。当前持仓展示缓存流动性、市值、`当前价格 / 买入价格` 倍数与 Token 复制。交易历史使用后端真分页、时间筛选，并展示买入/平仓时间和人类可读原因；
- `/signals`：产品名称“样本采集”，页面标题“样本预览”；展示模型分/入选线、入选、成熟标签、launchpad 与算法全称模型名，并提供一键导出全部 mature/tagged 样本为 `meme数据.csv`；
- `/runtime`：TrainingWorker、collector/monitor-only、model health、scheduler、reconciliation/liquidation 和实盘两步接口状态；scheduler 状态包含 `schedule_mode / next_entry_freeze_at / next_training_at / model_entries_paused / pending_activation_run_id`；
...
- 采集生命周期：monitor-only 不调用 discovery，但继续 paper exit 和 T+1h finalization；
- 时序：不足/达到 120 天、最后 20%/30 天、1h gap、无随机切分；
- Top 3 ML：扩展候选池、optional candidate skipped、单一决策线、`6TP-FP`/E-G-S、自适应 4..N 特征数、fold-train-only 排序、one-standard-error 奥卡姆、final holdout 隔离、`E_exec` 影子指标权重恒为 0；
...
- 模拟：四策略同 session、独立账本事务、费用/滑点/延迟/失败、每策略 10 仓、同币跨策略/多批次、1m first-touch、实时 liquidity、Jupiter Token→USDC 只读 quote、quoted fill / explicit no-route / API unavailable fallback 三态、closing 失败重试/重启恢复、model generation history 隔离；
...
2. README 指定入场特征持续采集，`price` 默认训练、`launchpad` 仅保存为样本元数据且排除训练、`ln(liquidity_usd)` 采集但默认关闭并可在模型中心自选；
3. T+1h first-touch 标签完整落库，入场 `price_change_1h/5m` 不使用未来数据；
4. 扩展候选池 + 时序 gap + 每模型单一决策线 + `6TP-FP`/E/G/S（评分公式保持不变）+ 自适应 4..N / fold-train-only 特征选择 + one-standard-error 奥卡姆 + Top 3 独立工件；`E_exec` 只做权重为 0 的 shadow 指标；
...
8. 当前 simulation session 四策略 → Top 3 各自 prediction + `rules_only` 无预测开仓 → 1m first-touch → 刷新当前 liquidity → Jupiter Token→USDC read-only route quote（或明确 fallback）→ closing retry/restart → 四策略 PnL/模型审计/session history 的完整链；`model_1/2/3` 持仓/历史/审计按当前 model_id + selected_at 隔离，collector 关闭时仍可 monitor-only 退出；
9. Agent 只读上下文、持久化白名单 proposal、人工 approve/reject 和非实盘执行；live/wallet/secret 类 proposal 在创建阶段直接拒绝；
10. Dashboard/Models/Signals/Portfolio/Runtime/Agent 六页控制台；Top 3/E/G/S/四策略模型审计已展示；后端 115/115 pytest 与前端 production build 通过。
...
- 当前 `E = mean(clip((6TP-FP)/(6N+),-1,1))`、`G` 与 `S=0.60E+0.40G` 保持不变；新增 `E_exec` 仅作为 shadow/audit 指标，当前排名权重固定为 0。
