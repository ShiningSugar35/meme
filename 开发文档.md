...
| `backend/app/ml/` | 无泄漏特征、chronological split、扩展候选池、`5TP-FP`/E-G-S、fold-train-only 自适应特征维度、单一决策线、`E_exec` shadow 指标和 Top 3 工件。 |
...
| `backend/app/services/position_monitor.py` | 默认 3s、配置页可调的 current-price 持仓监控；GMGN 动态 Key 池 + 与 Collector 共享总 RPS、同 Token 合并；simulation Jupiter 按 Key 数受控并发 executable quote，live fail-closed 退出。 |
...
当前 `age` 准入自 2026-08-16 起进一步冻结为严格 `2 < age_minutes < 240`：在线 GMGN raw age 始终在 SafetyFilter 直接判断，Trenches transport boundary 同步使用 `min_created=2m`。2026-08-13 的历史迁移当时使用 `1 < age_minutes < 240`，相关备份与迁移统计仅作为 provenance 保留；历史 legacy CSV/export 的旧 `age` 是 `ln(age_minutes)`，CsvImporter 继续兼容该格式并还原 raw age 后执行**当前**边界，导入后统一规范为模型特征 `ln(age+1)`；当前新采集/导出同样使用 `ln(age+1)`。
...
上面的逐笔本金/return 只用于真实模拟执行回放；旧数据缺 raw liquidity 时不能伪造成逐笔真实 USD PnL。模型排名另用统一固定 $50 的理论收益：`profit_units = 5×TP - FP`，`fixed_profit_usd = 5×profit_units`。这允许 legacy binary tag 与新样本在同一分类经济尺度上比较，同时不改变真实资金曲线。

每个候选模型只搜索一个开发期 OOS 决策线。经济得分 `E` 使用各开发 fold 的 `clip((5TP-FP)/(5N+),-1,1)` 平均；泛化得分 `G` 综合 Average Precision 相对基准率的 AP Skill、fold 稳定性和时间衰减；综合分固定 `S=0.60E+0.40G`。Precision/Recall 作为审计指标保留，但不再以三档硬门槛驱动模型。
...
自动调度统一使用 `Asia/Shanghai`：正常状态按 `training_weekday`（当前周日）17:00 训练；`ModelHealthService` 为 `insufficient_data` 时自动切换为每天 17:00。每个到期点前 1 小时（16:00）先将 `model_entries_paused_for_rollover=true`，冻结 `model_1/2/3/rules_only` 四策略的新开仓，但已有仓位继续由默认 3s（可配置）持仓监控正常退出。17:00 到点直接创建 durable run，不等待持仓清零；关机错过时启动后按唯一 `scheduled_for` 创建 `startup_catchup`。计划失败只重试同一 run，不能通过创建新计划绕过冻结。

7 日 `ModelHealthService` 会分别统计三个 active model 的成熟 OOS prediction，并按近期 `5TP-FP` economic capture 与各模型训练时 final capture 比较，但从 v10 起只负责状态分类：`insufficient_data` 驱动每日 cadence，其他状态（含 `healthy/degraded`）使用周 cadence，不再直接创建绕过 16:00/17:00 生命周期的插队训练。候选训练完成后可跨重启等待四策略全部空仓；空仓后 `TrainingWorker` 激活新 Top 3，并以 `created_reason=model_generation_upgrade` 创建全新的 simulation session，四个策略统一回到 1000 USD 后再解除 entry gate。历史可加载的 retired Rank 1 人工回滚同样要求四策略先空仓，并以 `model_generation_rollback` 新建 session。
...
- `PositionMonitorWorker` 默认 `position_monitor_poll_seconds=4.0`，Key 3 专用 `/v1/token/info` 通道并发拉取持仓 Token 的当前 price/liquidity/decimals；同 Token 的模拟/实盘仓位合并成一次行情请求。专用 limiter 保守设为 15 req/s，低于 GMGN 当前 token-info 官方 20 req/s/weight=1 上限；循环实际耗时写入 `position_monitor_status.last_cycle_seconds`，不得用配置值伪装真实 cadence；
...
- Top 3 ML：扩展候选池、optional candidate skipped、单一决策线、`5TP-FP`/E-G-S、自适应 4..N 特征数、fold-train-only TreeSHAP interaction 排序、one-standard-error + 8% 相对 S 损失双门槛奥卡姆、8% 性能带内 model-family 多样性、final holdout correlation/Jaccard 只读审计与隔离、`E_exec` 影子指标权重恒为 0；
...
4. 扩展候选池 + 时序 gap + 每模型单一决策线 + `5TP-FP`/E/G/S+ 自适应 4..N / fold-train-only TreeSHAP interaction 特征排序 + one-standard-error 与 8% 相对 S 损失双门槛奥卡姆 + 8% 性能带内 model-family 多样性 Top 3 + final holdout correlation/Jaccard 审计；`E_exec` 只做权重为 0 的 shadow 指标；
...
8. 当前 simulation session 四策略 → Top 3 各自 prediction + `rules_only` 无预测开仓 → 独立 PositionMonitorWorker 默认 3s（可配置）拉 current price/liquidity → 当前价命中 TP/SL/timeout → Jupiter Token→USDC executable quote（或明确 fallback）→ closing retry/restart → 四策略 PnL/模型审计/session history 的完整链；重大模型换代先冻结四策略新开仓、全平后新建 simulation session；
...
- 2026-08-12 当时 `E = mean(clip((6TP-FP)/(6N+),-1,1))`；自 2026-08-14 起当前口径改为 `E = mean(clip((5TP-FP)/(5N+),-1,1))`，`G` 与 `S=0.60E+0.40G` 结构保持不变；新增 `E_exec` 仅作为 shadow/audit 指标，当前排名权重固定为 0。
...
- 模型经济代理从 gross 标签的 `+6/-1` 调整为 `+5/-1`：`U=5TP-FP=N+×recall×(6-1/precision)`，`E=mean(clip((5TP-FP)/(5N+),-1,1))`；`G` 与 `S=0.60E+0.40G` 结构不变。该 haircut 只改变阈值搜索、E/S、奥卡姆与 Top3 排名，不改变 H1 `1.6x/0.9x` 标签或模拟盘真实交易。ModelHealth 同样读取 `WIN_UNITS=5`，禁止训练与健康监控口径分裂。
...
- 生产 Trenches 请求现在只承担 transport/lifecycle 范围约束：`filters=[offchain,onchain]`、`launchpad_platform_v2=true`、冻结的 8 个业务 Launchpad、Solana `quote_address_type=[4,5,3,1,13,0]`、当前 `min_created=2m`、`max_created=240m` 和 `limit<=80`。官方平台目录后续虽已扩展，但本系统的 8 平台属于冻结业务边界，本次没有借接口升级扩大样本范围。
...
### 2026-08-16 Event2m / Market Regime / Adaptive Policy generation

- 样本准入统一从严格 `age>1m` 调整为严格 `age>2m`，GMGN Trenches transport boundary 同步为 `min_created=2m`。正式 feature generation 固定为 `event2m_regime_v2`；`legacy_pre_event_v1` 与开发热重载期间产生的 `event2m_regime_v1` 试运行样本均保留原文、标签、预测和成交审计，但 `SampleRepository.list_mature/statistics`、标签队列、训练、Model Health 与 Regime 策略收益全部只读取当前 generation。由于 v1 已产生 23 个关联历史模拟仓位，未做破坏性删除，而是整代封存；v2 在物理备份 `data/backups/meme_quant_pre_event2m_20260816T082356Z.db` 后从 0 样本重新开始。禁止用旧样本缺失列作零填充或 hindsight backfill。
- 对现有 31 特征逐项去重后，新增候选仅补信息空缺：`price_change_2m`、1m volume/swaps、1m buy/sell count 与 volume imbalance、1m 平均成交额、由两根已闭合 1m bar 构造的 `volume_2m` / `volume_acceleration_2m`、holder/age、marketcap、creator status、GMGN 集成的 DexScreener Ads/Boost/TrendingBar、X follower、TG call。`fresh_wallet_rate` 即现有新钱包占比，smart/renowned/top wallet、holder/dev/sniper/bot、5m/1h momentum 等均不重复开发。可选 catalog 共 48 个，默认训练候选为除 `ln(liquidity_usd)` 外的 47 个；新增 Event2m 字段只是获得参加 fold-train-only chronological OOS、one-SE + 8% 奥卡姆选择和 final holdout certification 的资格，并不等于自动进入最终生产模型。`ln(liquidity_usd)` 继续作为 opt-in 候选，待同代覆盖充分后再开启。`feature_snapshot_at` 是入场特征冻结点；T+1h 标签链只写 target/price-window 事实，不再补写入场特征。
- Token 局部 2m 与 Market Regime lookback 严格分层：Token 事件不能越过 sample `entry_time`；SOL 5m/15m/1h、Solana network、Meme breadth、Attention、Execution、Strategy Performance 可以使用更长过去窗口，只要 observation time<=decision time。Market Regime snapshot 持久化原始 family features、score/label/confidence、source health 与 reasons。
- Provider topology：GMGN 继续作为 Token/Trending/Hot Search/Smart Money/KOL/Dex promotion 主源；DEX Screener Public 只在 GMGN aggregate attention 不可用或 Dex promotion 子家族缺失时低频补充 `/ads/latest/v1` 与 `/token-boosts/latest/v1`，不会成为 per-token 依赖。Solana Network 只采 `getRecentPerformanceSamples + getRecentPrioritizationFees`，按 4 个独立 Alchemy Free account 轮转，失败后转 2 个 Ankr Freemium HTTPS，最后才是 Solana Public emergency；Public source 会降低 Regime confidence。
...
- Ankr 官方 2026-07-28 文档确认 Freemium 每月 200M credits、Solana HTTPS 500 credits/request、约 1800 req/min guaranteed、Freemium 仅 HTTPS。运行时对 HTTP 异常做 provider-label/status 归一化，绝不把包含 API token 的 URL 写入 runtime/log/audit。2026-08-16 最终两轮只读现场健康探针中，两个本机 Ankr 项目对 `getSlot / getRecentPerformanceSamples / getRecentPrioritizationFees` 均返回 HTTP 403；本地 Key 均为普通 API Key，endpoint 构造也与 Ankr 当前 `/solana/<API_KEY>` 官方格式一致，因此记录为外部项目/授权未生效，而不是代码 URL 拼接故障。任何 Ankr 失败均不能阻断 Alchemy 主路径，运行时继续转入 Solana Public emergency 并降低 Regime confidence。
- Adaptive Policy 使用三个动作 `DEFENSIVE / NEUTRAL / EXPANSIVE`，分别对每个模型自己的 base threshold 施加有限 logit offset；`rules_only` 永不调用 adaptive。三个模型每次仍记录 neutral counterfactual。V2 默认强制 shadow-only：Regime 即使推荐非 Neutral，实际 action 仍为 Neutral，直到同一 feature generation 至少 120 个独立 sample cluster、至少 40 个 policy-separating cluster，chronological 70/30 development/certification 的 +5/-1 shadow excess utility 满足 development mean>0、certification 95% LCB>0 且三个模型 certification mean 均>=0。5% exploration 另需 `adaptive_exploration_ready=true` 第二道持久化证据门，因此仅改环境变量不能提前在线探索。
