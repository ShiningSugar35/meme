...
第一模型可作为 bootstrap Champion 供模拟/影子验证，但这不等于已经证明真实美元收益优势。已有 Champion 后，系统按 incumbent 原 algorithm + feature schema 在当前 pre-holdout 历史上重建 evaluation bundle，再与 Challenger 在完全相同 final OOS rows 比较。默认 31 特征下，严格泛化研究的 robust AP 以 ExtraTrees 略高；生产五候选按正式 OOS utility、35% Precision 门槛和稳定性规则选择 Random Forest 为 Champion，最终时间窗 Precision=39.74%、Recall=32.29%、78 trades。
