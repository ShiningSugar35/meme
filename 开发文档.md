...
- 当前样本准入冻结为严格 `2 < age_minutes < 300`，GMGN Trenches transport boundary 同步为 `min_created=2m / max_created=300m`。正式 feature generation 已切换为 `event1m_regime_v3`；`legacy_pre_event_v1`、`event2m_regime_v1` 与 `event2m_regime_v2` 均保留原文、标签、预测和成交审计，但 `SampleRepository.list_mature/statistics`、训练、Model Health 与 Adaptive/Regime 策略证据只读取当前 generation。切换时 v2 仅有 4 条 pending、0 个关联仓位，因此不做破坏性删除；这些旧 pending 仍可按既有 T+1h 链路自然补标，但不得进入 v3 训练或事后回填新特征。
