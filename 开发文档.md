...
`age` 自 2026-08-13 起正式冻结为严格 `1 < age_minutes < 240`：在线 GMGN raw age 在 SafetyFilter 直接判断；legacy CSV/export 的 `age` 是 `ln(age_minutes)`，CsvImporter 在 log 空间执行等价边界。该 age 规则已追溯迁移当前真实库：备份 `data/backups/meme_quant_pre_age240_20260813T0318Z.db`，删除 497 个超龄/不合规样本及其 201 predictions、45 simulation positions、88 trades；当时迁移后保留 1856 条 mature H1 v4 样本（344 正 / 1512 负），age 违规数为 0。

同日后续筛选再次冻结为 `0.125 <= top_10_holder_rate <= 0.28` 与 `volume_1h / swaps_1h > 31`：Trenches 使用 `min_top_holder_rate=0.125 / max_top_holder_rate=0.28`，local SafetyFilter 与 legacy `量化训练采集.py` 使用同一边界；数据库/CSV 中 `volume_1h/swaps_1h` 是原始成交均额的自然对数，因此 CsvImporter 在 log 空间执行等价的 `feature > ln(31)`，避免旧导出重新灌入不合格样本。迁移前备份 `data/backups/meme_quant_pre_filter_top028_vps31_20260813T050755Z.db`；从 1858 条样本中删除 182 条不合格样本（165 mature negative、17 mature positive），删除其 36 条 predictions，并仅解绑 4 个已关闭 simulation positions 的 sample/prediction 引用，保留 8 条历史成交事实与账户流水。迁移后保留 1676 条样本：1674 mature（327 正 / 1347 负）+ 2 pending，新筛选违规数为 0。
...
10. Dashboard/Models/Signals/Portfolio/Runtime/Agent 六页控制台；Top 3/E/G/S/四策略模型审计已展示；后端 131/131 pytest 与前端 production build 通过。
