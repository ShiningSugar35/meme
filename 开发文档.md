...
`top_10_holder_rate` 准入阈值自 2026-08-13 起统一为闭区间 `[0.125, 0.275]`：Trenches 请求使用 `min_top_holder_rate=0.125 / max_top_holder_rate=0.275`，本地 SafetyFilter 使用相同边界。该规则只影响后续新增样本，历史已入库样本不做追溯清理或重标。
...
10. Dashboard/Models/Signals/Portfolio/Runtime/Agent 六页控制台；Top 3/E/G/S/四策略模型审计已展示；后端 120/120 pytest 与前端 production build 通过。
