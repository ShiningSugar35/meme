...
| POST | `/api/simulation/reset` | 无模拟持仓时关闭旧 session，并创建四个各自 `1000 USD + 0.1 SOL` 的策略账本。 |
...
`TrainingService` 在一次 durable run 内注册三个模型工件，并通过 `active_model_slots` 同事务替换当前 Top 3。发布前必须满足：
...
7 日 `ModelHealthService` 会分别统计三个 active model 的成熟 OOS prediction，并按近期 `6TP-FP` economic capture 与各模型训练时 final capture 比较；样本不足时仅报告，不凭少量交易触发重训。若任一 active model 的近期 capture 低于其基线的配置比例（默认 70%），只创建一个 durable `degraded` training run，并有 24 小时冷却；新 run 仍按完整候选池 + chronological OOS + E/G/S 重建 Top 3。历史可加载的 retired Rank 1 支持人工恢复到 slot 1。
...
- 每个 simulation session 创建 `model_1`、`model_2`、`model_3`、`rules_only` 四个独立策略账本，各初始化 `1000 USD + 0.1 SOL`；schema v8 中 `positions.account_kind` 统一为 `simulation`，具体策略只看 `strategy_key`。reset 只关闭旧 session/创建新 session，不删除历史仓位、成交和 PnL；
...
- prediction 不使用成熟 `tag` 事后“替模拟盘成交”；理论标签收益与 executable simulation PnL 完全分离；
...
- `/`：数据集、实盘累计已实现损益、Top 3、四策略模拟收益、信号和 worker 总览。Rank 1/2/3 前端显示 `YYYYMMDD-xxx` 短名，内部唯一 model id/version 不改变；
- `/portfolio`：持仓主工作台。页面先在 `simulation/live` 两种模式间切换；simulation 下再选择 `model_1 / model_2 / model_3 / rules_only`。四张策略卡展示模型短名/不用模型、现金、SOL 储备、持仓和已实现 PnL；当前持仓展示缓存流动性、市值、`当前价格 / 买入价格` 倍数与 Token 复制。交易历史使用后端真分页、时间筛选，并展示买入/平仓时间和人类可读原因，失败平仓时间显示“卖出失败”。交易审计第一列为“模型”，按四策略独立列示实际仓位时间/交易数/PnL；live 模式使用同构 UI 与 `account_kind='live'` 账本，但不伪造钱包余额，也不代表自动 live BUY 已接通；
- `/signals`：产品名称“样本采集”，页面标题“样本预览”；展示模型分/入选线、入选、成熟标签、launchpad 与短模型名，并提供一键导出全部 mature/tagged 样本为 `meme数据.csv`；
- `/runtime`：TrainingWorker、collector/monitor-only、model health、scheduler、reconciliation/liquidation 和实盘两步接口状态；
- `/models`：Top 3、E/G/S、单一决策线、final certification、`rules_only` 基线、候选池 skipped 状态、feature coverage/自选候选特征、durable training runs、版本拒绝与 Rank 1 回滚；
