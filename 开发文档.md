...
`top_10_holder_rate` 准入阈值自 2026-08-13 起统一为闭区间 `[0.125, 0.275]`：Trenches 请求使用 `min_top_holder_rate=0.125 / max_top_holder_rate=0.275`，本地 SafetyFilter 使用相同边界。`age` 同日正式冻结为严格 `1 < age_minutes < 240`：在线 GMGN raw age 在 SafetyFilter 直接判断；legacy CSV/export 的 `age` 是 `ln(age_minutes)`，CsvImporter 在 log 空间执行等价边界。该 age 规则已追溯迁移当前真实库：备份 `data/backups/meme_quant_pre_age240_20260813T0318Z.db`，删除 497 个超龄/不合规样本及其 201 predictions、45 simulation positions、88 trades；迁移后保留 1856 条 mature H1 v4 样本（344 正 / 1512 负），违规数为 0。
