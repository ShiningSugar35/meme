...
10. Dashboard/Models/Signals/Portfolio/Runtime/Agent/Configuration 七页控制台；Top 3/E/G/S/四策略模型审计已展示；后端 142/142 pytest 与前端 production build 通过。
...
- 新增真实 adapter smoke 与回归：CatBoost/FLAML 均完成 `build_pipeline → fit_pipeline → predict_proba`；FLAML unit smoke 验证 `time/holdout`、概率输出、joblib 保存/重载与非等权拒绝。当前全量后端测试为 142/142，前端 production build 通过。
