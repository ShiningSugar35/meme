...
- 生产 Trenches 请求现在只承担 transport/lifecycle 范围约束：`filters=[offchain,onchain]`、`launchpad_platform_v2=true`、冻结的 8 个业务 Launchpad、Solana `quote_address_type=[4,5,3,1,13,0]`、`min_created=1m`、`max_created=240m` 和 `limit<=80`。官方平台目录后续虽已扩展，但本系统的 8 平台属于冻结业务边界，本次没有借接口升级扩大样本范围。
