...
- binary-v3 标签：先触及 0.9x 或 2h 内未先触及 1.6x 均为 `tag=0`；先触及 1.6x 为 `tag=1`；2h 最终收盘仅作审计，不参与标签判断。
- 同一 1m K 线 TP/SL 同时触发时，`first_sl <= first_tp`，止损优先。
- 历史 CSV 的 16 条非 TP 旧正类在 binary-v3 中统一重标为 `tag=0`；原 2h max/min/final-close 事实继续保留用于审计。
...
| GET | `/api/samples/export.csv` | 导出数据库全部 `mature + tag` 样本为 UTF-8 BOM `meme数据.csv`，包含 launchpad、特征、2h 审计事实、binary tag 与标签版本。 |
...
每个算法按预排序 feature importance 比较 12 / 20 / 全量特征，使用 one-standard-error 规则选择仍与算法最佳分统计上近似等价的最小子集。随后只按开发期 `S` 选 Top 3；最终 holdout 在选择完成后才附加到审计指标，绝不进入 Top 3 排序。Top 3 各自以选定特征和单一决策线生成独立 bundle。
...
- 每个 simulation session 创建 `model_1`、`model_2`、`model_3`、`rules_only` 四个独立策略账本，各初始化 `1000 USD`；schema v10 继续保持单一 USD 会计，并新增 model generation 边界：`active_model_slots.selected_at` 是当前模型上线统计起点。自动/人工换模只有在三模型策略全部空仓后发生，届时 `model_1/2/3` 各自重新初始化为 `1000 USD`，旧 generation 的 position/trade/PnL 保留审计但不进入新卡片统计；`rules_only` 是连续无模型基线；
...
- `COLLECTOR_ENABLED=false` 但 `PAPER_MARKET_MONITOR_ENABLED=true` 时运行 monitor-only worker，已有模拟仓位退出和 T+2h label finalization 不依赖 discovery 是否开启；Collector/monitor 周期仍会刷新模拟手续费所需的 SOL/USD 事实。
...
- 标签：SL 先、TP 先、同 bar、2h timeout 无论最终 close 如何均为负类、窗口边界、first-touch/return source 落库；
...
1. 2319 条 legacy CSV 幂等迁入 SQLite；binary-v3 重标后 timeout-positive 全部 tag0，标签版本和原 2h 审计事实保持可追溯；
