...
`age` 准入自 2026-08-13 起正式冻结为严格 `1 < age_minutes < 240`：在线 GMGN raw age 始终在 SafetyFilter 直接判断；历史 legacy CSV/export 的旧 `age` 是 `ln(age_minutes)`，CsvImporter 兼容该格式并还原 raw age 做边界判断，导入后统一规范为模型特征 `ln(age+1)`；当前新采集/导出同样使用 `ln(age+1)`。该 age 规则已追溯迁移当前真实库：备份 `data/backups/meme_quant_pre_age240_20260813T0318Z.db`，删除 497 个超龄/不合规样本及其 201 predictions、45 simulation positions、88 trades；当时迁移后保留 1856 条 mature H1 v4 样本（344 正 / 1512 负），age 违规数为 0。
