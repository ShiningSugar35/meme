...
- 研究对象：允许清单内的 Solana Launchpad 新创建 Token，以及仍处于内盘阶段、按 5m volume 排序的 GMGN Trending 候选。
- 当前特征代：`event1m_regime_v10`。v4–v9 仅保留历史审计，不进入当前训练；v10 在 v9 的 New + Volume 来源合同上把 Trending 固定为 `order_by=volume, interval=5m`，并继续禁止 near-completion 进入正式样本。
