...
const CURRENT_ECONOMIC_OBJECTIVE = "precision_recall_payoff_v4";
const isCurrentExpectedReturn = (model: ModelVersion) => model.metrics.economic_objective_version === CURRENT_ECONOMIC_OBJECTIVE;
...
const algorithmNames: Record<string, string> = { logistic_regression: "Logistic Regression", decision_tree: "Decision Tree", hist_gradient_boosting: "Histogram Gradient Boosting", gradient_boosting: "Gradient Boosting", ada_boost: "AdaBoost", extra_trees: "Extra Trees", random_forest: "Random Forest", rbf_svm: "RBF SVM", xgboost: "XGBoost", lightgbm: "LightGBM", catboost: "CatBoost", tabpfn: "TabPFN", flaml_automl: "FLAML AutoML" };
...
export function ModelsPage() {
  ...
  const currentActiveModels = activeModels.filter(isCurrentExpectedReturn);
  ...
  const diversity = latestCompleted ? asRecord(asRecord(latestCompleted.summary).diversity) : {};
  const diversityPairs = asArray(diversity.pairs);
  const diversityFamilies = asRecord(diversity.families);

  return <div className="page-stack">
    <section className="page-heading"><div><p className="eyebrow">MODEL REGISTRY</p><h1>模型中心</h1><p>候选模型统一在时间外开发折上评估，按 Precision、Recall 与当前 +3/-1 盈亏比导出的期望收益 J 选出 Top 3；最终时间留出只作部署否决与审计。</p></div><div className="heading-actions"><button className="button button-primary" disabled={training || savingFeatures || Boolean(activeRun) || !activeFeatures.length} onClick={() => void train()}><RefreshCw size={17} className={training || activeRun?.status === "running" ? "spinning" : ""} />{activeRun ? `训练任务${activeRun.status === "running" ? "执行中" : "排队中"}` : training ? "提交中…" : `重新评估 Top 3`}</button></div></section>
    {notice && <div className="inline-notice">{notice}</div>}

    <section className="panel table-panel"><div className="panel-heading"><div><h2>当前 Top 3</h2><p>J = Recall × (4 - 1 / Precision) = (3TP - FP) / 正类数；盈亏平衡 Precision = 25%</p></div><StatusBadge tone={currentActiveModels.length === 3 ? "blue" : "orange"} label={currentActiveModels.length === 3 ? "3/3 当前合同" : `${currentActiveModels.length}/3 当前合同`} /></div>
      {activeModels.length ? <div className="table-scroll"><table><thead><tr><th>排名</th><th>模型</th><th>特征</th><th>决策线</th><th>Precision</th><th>Recall</th><th>理论收益</th><th>期望收益 J</th></tr></thead><tbody>{activeModels.map((model) => <tr key={model.id}><td><StatusBadge tone={model.active_slot === 1 ? "gold" : "blue"} label={`Top ${model.active_slot ?? "—"}`} /></td><td><strong>{modelLabel(model)}</strong><span className="table-sub">{algorithmName(model.algorithm)}</span></td><td>{model.feature_names.length}</td><td className="mono">{typeof model.active_threshold === "number" ? model.active_threshold.toFixed(3) : model.thresholds.decision?.toFixed(3) ?? "—"}</td><td>{pct(model.metrics.precision)}</td><td>{pct(model.metrics.recall)}</td><td>{money(model.metrics.fixed_profit_usd)}</td><td><strong>{isCurrentExpectedReturn(model) ? score(model.metrics.composite_score ?? model.metrics.economic_score ?? model.active_composite_score) : "历史口径"}</strong></td></tr>)}</tbody></table></div> : <EmptyState title="Top 3 尚未生成" detail="启动一次新训练后，系统会从候选池中选出三个模型。" />}
    ...
    <section className="info-strip"><Bot size={19} /><div><strong>评分与奥卡姆原则</strong><span>唯一主目标 J = Recall × (4 - 1 / Precision) = (3TP - FP) / 正类数；阈值在全部开发期时间外 operating points 中选择 J 最大者。AP、AUC、稳定性、衰减和模型家族多样性只做审计，不参与 Top 3 排名。最终留出集只作部署否决，不反向调阈值。</span></div></section>
  </div>;
}
