...
const isCurrentExpectedReturn = (model: { metrics: { economic_objective_version?: string | null } } | null | undefined) => model?.metrics.economic_objective_version === CURRENT_ECONOMIC_OBJECTIVE;
...
export function DashboardPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="metric-grid">
      ...
      <MetricCard title="Rank 1 期望收益 J" value={isCurrentExpectedReturn(rank1) ? score(rank1?.metrics.composite_score ?? rank1?.active_composite_score) : "合同已失效"} context={`Precision ${percent(typeof rank1?.metrics.precision === "number" ? rank1.metrics.precision : null)} · Recall ${percent(typeof rank1?.metrics.recall === "number" ? rank1.metrics.recall : null)}`} icon={Activity} tone="blue" />
    ...
    <section className="panel table-panel"><div className="panel-heading"><div><h2>Top 3 模型</h2><p>训练排名只使用开发期时间外期望收益 J；最近最终留出只作部署否决与审计。</p></div></div>{data.active_models.length ? <div className="table-scroll"><table><thead><tr><th>排名</th><th>模型</th><th>特征</th><th>决策线</th><th>Precision</th><th>Recall</th><th>理论收益</th><th>期望收益 J</th></tr></thead><tbody>{[...data.active_models].sort((a, b) => Number(a.active_slot ?? 99) - Number(b.active_slot ?? 99)).map((model) => <tr key={model.id}><td><StatusBadge tone={model.active_slot === 1 ? "gold" : "blue"} label={`Top ${model.active_slot}`} /></td><td>{data.strategies.find((item) => item.model_id === model.id)?.label ?? model.algorithm}</td><td>{model.feature_names.length}</td><td>{typeof model.active_threshold === "number" ? model.active_threshold.toFixed(3) : "—"}</td><td>{percent(typeof model.metrics.precision === "number" ? model.metrics.precision : null)}</td><td>{percent(typeof model.metrics.recall === "number" ? model.metrics.recall : null)}</td><td>{money(typeof model.metrics.fixed_profit_usd === "number" ? model.metrics.fixed_profit_usd : null)}</td><td><strong>{isCurrentExpectedReturn(model) ? score(model.metrics.composite_score ?? model.metrics.economic_score) : "历史口径"}</strong></td></tr>)}</tbody></table></div> : <EmptyState title="Top 3 尚未生成" detail="完成一次新训练后显示。" />}</section>
