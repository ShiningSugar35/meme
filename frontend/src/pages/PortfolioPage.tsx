...
const beijingTime = (value: string | null | undefined) => {
  if (!value) return "—";
  return new Intl.DateTimeFormat("zh-CN", { timeZone: "Asia/Shanghai", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit", hourCycle: "h23" }).format(new Date(value));
};
...
export function PortfolioPage() {
  ...
  const resetSimulation = async () => { if (!window.confirm("确认开始新的模拟会话？历史记录会保留，四个策略仅重置实验统计，不设置本金或现金余额。")) return; setBusy(true); try { await api.resetSimulation(); setNotice("新的四策略模拟会话已创建"); setPage(1); await refresh(); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "模拟会话重置失败"); } finally { setBusy(false); } };
