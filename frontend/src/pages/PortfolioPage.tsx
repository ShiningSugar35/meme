...
const failureReasonName: Record<string, string> = { no_route: "无可用卖出路由", quote_failed: "卖出报价失败", network: "网络请求失败", api: "交易 API 失败", rate_limit: "接口限流", chain_rejected: "链上拒绝", insufficient_funds: "余额不足", risk_rejected: "风控拒绝", paper_sol_reserve_depleted: "SOL 手续费储备不足", order_failed: "链上订单失败", order_expired: "订单过期" };
...
export function PortfolioPage() {
  ...
  const resetSimulation = async () => { if (!window.confirm("确认开始新的模拟会话？历史记录会保留，四个策略账户都会重置为 $1000 + 0.1 SOL。")) return; setBusy(true); try { await api.resetSimulation(); setNotice("新的四策略模拟会话已创建"); setPage(1); await refresh(); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "模拟会话重置失败"); } finally { setBusy(false); } };
  ...
  return <div className="page-stack">
    ...
    <section className="panel table-panel"><div className="panel-heading"><div><h2>交易历史</h2><p>已完成平仓的历史记录，计入滑点和平台费。</p></div><StatusBadge label={`${view.history.total} 条`} /></div><div className="history-toolbar"><label>开始时间<input type="datetime-local" value={startDraft} onChange={(e) => setStartDraft(e.target.value)} /></label><label>结束时间<input type="datetime-local" value={endDraft} onChange={(e) => setEndDraft(e.target.value)} /></label><button className="button button-secondary" onClick={applyTimeFilter}>筛选</button>{(startAt || endAt) && <button className="button button-secondary" onClick={clearTimeFilter}>清除</button>}<label className="page-size-control">每页<select value={pageSize} onChange={(e) => updatePageSize(Number(e.target.value))}>{pageSizeOptions.map((size) => <option value={size} key={size}>{size} 行</option>)}</select></label></div>
