...
class Repositories:
    def __init__(self, db: aiosqlite.Connection):
        self.db = db
        # whether this repository instance owns the DB connection and must close it
    ...
    async def close(self):
        """Close the underlying aiosqlite connection if owned."""
        if getattr(self, "_closed", False):
            return
        try:
            # ensure no background operations remain
            await self.db.close()
    ...
    # provider_requests
    async def append_provider_request(self, provider: str, endpoint: str, method: str, status_code: Optional[int], latency_ms: Optional[int], ok: bool, error_code: Optional[str], error_summary: Optional[str], request_summary_json: Optional[str], response_summary_json: Optional[str]):
        created_at = datetime.now(timezone.utc).isoformat()
        try:
            await self.db.execute("BEGIN")
            await self.db.execute(
                ...
                (provider, endpoint, method, status_code, latency_ms, 1 if ok else 0, error_code, error_summary, request_summary_json, response_summary_json, created_at),
            )
            await self.db.commit()
        except Exception as e:
            logger.error("append_provider_request failed", error=str(e))
    ...
    # strategy_groups
    async def create_strategy_group(self, name: str, x: float, y: float, t_seconds: int, is_live: bool = False, priority: int = 100, raw_config_json: str = "{}") -> int:
        created_at = datetime.now(timezone.utc).isoformat()
        try:
            await self.db.execute("BEGIN")
            ...
            await self.append_system_event("ERROR", "DB", "create_strategy_group failed", str({"error": str(e), "name": name}))
            raise
    ...
    async def update_strategy_group(self, id: int, updates: Dict[str, Any]):
        ...
        # note: updated_at param appended earlier to params? We'll set updated_at now
        ...
        try:
            ...
            await self.append_system_event("ERROR", "DB", "update_strategy_group failed", str({"error": str(e), "id": id}))
            raise
    ...
    async def increment_config_version(self, id: int):
        try:
            await self.db.execute("BEGIN")
            await self.db.execute("UPDATE strategy_groups SET config_version = config_version + 1, updated_at = ? WHERE id = ?", (datetime.now(timezone.utc).isoformat(), id))
            await self.db.commit()
        except Exception as e:
            await self.append_system_event("ERROR", "DB", "increment_config_version failed", str({"error": str(e), "id": id}))
            raise
    ...
    async def upsert_token_first_seen(self, token_mint: str, chain: str = 'solana', pool_address: Optional[str] = None, launchpad: Optional[str] = None, symbol: Optional[str] = None, name: Optional[str] = None, pool_created_at: Optional[str] = None, latest_state: str = 'discovered'):
        ...
        try:
            ...
            # if exists, do not overwrite first_seen_at
            await self.db.commit()
        except Exception as e:
            await self.append_system_event("ERROR", "DB", "upsert_token_first_seen failed", str({"error": str(e), "token": token_mint}))
            raise

    async def update_token_latest_snapshot(self, token_mint: str, latest_snapshot_id: int, latest_price_usd: Optional[float] = None, latest_price_sol: Optional[float] = None, latest_liquidity_usd: Optional[float] = None, latest_sol_side_liquidity: Optional[float] = None, latest_market_cap: Optional[float] = None, latest_type: Optional[str] = None):
        now = datetime.now(timezone.utc).isoformat()
        try:
            await self.db.execute("BEGIN")
            await self.db.execute(
                ...
                (latest_snapshot_id, latest_price_usd, latest_price_sol, latest_liquidity_usd, latest_sol_side_liquidity, latest_market_cap, latest_type, now, token_mint),
            )
            await self.db.commit()
        except Exception as e:
            await self.append_system_event("ERROR", "DB", "update_token_latest_snapshot failed", str({"error": str(e), "token": token_mint}))
            raise
    ...
    async def insert_token_metric_snapshot(self, token_mint: str, observed_at: str, raw_json: str, **kwargs):
        ...
        try:
            ...
            await self.append_system_event("ERROR", "DB", "insert_token_metric_snapshot failed", str({"error": str(e), "token": token_mint}))
            raise

    async def get_latest_token_metric_snapshot(self, token_mint: str) -> Optional[Dict[str, Any]]:
        async with self.db.execute("SELECT * FROM token_metric_snapshots WHERE token_mint = ? ORDER BY observed_at DESC LIMIT 1", (token_mint,)) as cur:
            row = await cur.fetchone()
        return dict(row) if row else None

    async def list_token_metric_snapshots(self, token_mint: str, limit: int = 100) -> List[Dict[str, Any]]:
        cur = await self.db.execute("SELECT * FROM token_metric_snapshots WHERE token_mint = ? ORDER BY observed_at DESC LIMIT ?", (token_mint, limit))
        rows = await cur.fetchall()
        return [dict(r) for r in rows]

    # kline_snapshots
    async def insert_kline_snapshots(self, token_mint: str, interval: str, open_time: str, open_p: float, high: float, low: float, close: float, buy_volume: float, sell_volume: float, volume_usd: float, raw_json: str):
        try:
            await self.db.execute("BEGIN")
            await self.db.execute(
            ...
            await self.append_system_event("ERROR", "DB", "insert_kline_snapshots failed", str({"error": str(e), "token": token_mint}))
            raise

    async def get_recent_klines(self, token_mint: str, interval: str, limit: int = 10) -> List[Dict[str, Any]]:
        async with self.db.execute("SELECT * FROM kline_snapshots WHERE token_mint = ? AND interval = ? ORDER BY open_time DESC LIMIT ?", (token_mint, interval, limit)) as cur:
            rows = await cur.fetchall()
        return [dict(r) for r in rows]

    # tick_snapshots
    async def insert_tick_snapshot(self, token_mint: str, source: str, observed_at: str, price_usd: float, price_sol: float, liquidity_usd: float, sol_side_liquidity: float, market_cap: float, raw_json: Optional[str] = None):
        try:
            await self.db.execute("BEGIN")
            await self.db.execute(
            ...
            await self.append_system_event("ERROR", "DB", "insert_tick_snapshot failed", str({"error": str(e), "token": token_mint}))
            raise

    # discovery_events
    def normalize_pool_address(self, pool_address: Optional[str]) -> str:
        """Normalize pool_address: None -> ''"""
    ...
    async def get_discovery_event_by_snapshot_token_pool(
        ...
        """
        Get discovery event by snapshot_id + token_mint + pool_address.
        
        Returns:
            Discovery event row if found, None otherwise.
        """
    ...
    async def create_discovery_event_idempotent(
        ...
        """
        Create discovery event, but idempotent for same (snapshot_id, token_mint, pool_address).
        
        Args:
            token_mint, pool_address, pool_created_at, t_seconds, snapshot_id
            
        Returns:
            (discovery_event_id, created: bool)
            - created=True: newly created
            - created=False: already existed (return existing)
        """
        # If snapshot_id is None, just create (old behavior)
        if snapshot_id is None:
        ...
        try:
            ...
            # May be UNIQUE constraint failure (concurrent insert)
            await self.append_system_event(
                "WARN", "DB", "create_discovery_event_idempotent failed, checking existing",
                str({"error": str(e), "token": token_mint, "snapshot_id": snapshot_id})
            )
            # Check again
            existing = await self.get_discovery_event_by_snapshot_token_pool(
                snapshot_id, token_mint, pool_address
    ...
    async def create_discovery_event(
        ...
        try:
            ...
            await self.append_system_event("ERROR", "DB", "create_discovery_event failed", str({"error": str(e), "token": token_mint}))
            raise
    ...
    async def get_latest_discovery_event_for_token(self, token_mint: str) -> Optional[Dict[str, Any]]:
        """Get the latest discovery event for a token (may be open or closed)."""
        async with self.db.execute(
    ...
    async def get_recent_ticks(self, token_mint: str, seconds: int = 60) -> List[Dict[str, Any]]:
        # naive: return last N by observed_at within approximate seconds; assumes ISO timestamps
        async with self.db.execute("SELECT * FROM tick_snapshots WHERE token_mint = ? ORDER BY observed_at DESC LIMIT ?", (token_mint, 1000)) as cur:
            rows = await cur.fetchall()
        return [dict(r) for r in rows]
    ...
    async def insert_strategy_match(
        ...
        try:
            ...
            await self.db.execute(
                ...
                (token_mint, strategy_id, strategy_config_version, snapshot_id, discovery_event_id, stage, 1 if passed else 0, pass_fail_detail_json, feature_vector_json, created_at),
            )
            await self.db.commit()
        except Exception as e:
            await self.append_system_event("ERROR", "DB", "insert_strategy_match failed", str({"error": str(e), "token": token_mint}))
            raise

    async def list_token_strategy_matches(self, token_mint: str, limit: int = 100) -> List[Dict[str, Any]]:
        async with self.db.execute("SELECT * FROM token_strategy_matches WHERE token_mint = ? ORDER BY id DESC LIMIT ?", (token_mint, limit)) as cur:
            rows = await cur.fetchall()
        return [dict(r) for r in rows]
    ...
    async def create_position(
        ...
        try:
            ...
            await self.append_system_event("ERROR", "DB", "create_position failed", str({"error": str(e), "token": token_mint}))
            raise
    ...
    async def update_position_remaining(self, position_id: int, remaining_token_amount: float, remaining_value_usd: float, last_fill_at: Optional[str] = None, last_fill_price_usd: Optional[float] = None):
        try:
            await self.db.execute("BEGIN")
            await self.db.execute("UPDATE positions SET remaining_token_amount = ?, remaining_value_usd = ?, last_fill_at = ?, last_fill_price_usd = ? WHERE id = ?", (remaining_token_amount, remaining_value_usd, last_fill_at, last_fill_price_usd, position_id))
            await self.db.commit()
        except Exception as e:
            await self.append_system_event("ERROR", "DB", "update_position_remaining failed", str({"error": str(e), "position_id": position_id}))
            raise

    async def close_position(self, position_id: int, closed_at: Optional[str] = None, close_reason: Optional[str] = None, total_return_sol: Optional[float] = None):
    ...
    async def get_open_live_position_by_token_and_cycle(
        ...
        """
        Get open live position for token in a specific cycle.
        
        Args:
            token_mint: Token mint address
            discovery_event_id: Cycle ID. If None, check for any open live position (legacy).
        ...
        if discovery_event_id is not None:
            # Check for open live position with specific discovery_event_id
            async with self.db.execute(
            ...
            # Legacy: check for any open live position (for backward compatibility)
            async with self.db.execute(
    ...
    async def list_positions_by_token_and_is_live(self, token_mint: str, is_live: bool, limit: int = 100) -> List[Dict[str, Any]]:
        async with self.db.execute("SELECT * FROM positions WHERE token_mint = ? AND is_live = ? ORDER BY id DESC LIMIT ?", (token_mint, 1 if is_live else 0, limit)) as cur:
            rows = await cur.fetchall()
        return [dict(r) for r in rows]
    ...
    async def append_trade_event(self, idempotency_key: str, **kwargs) -> Dict[str, Any]:
        ...
        async with self.db.execute("SELECT * FROM trade_events WHERE idempotency_key = ?", (idempotency_key,)) as cur:
            row = await cur.fetchone()
        if row:
            return dict(row)
        ...
        # support a whitelist of additional columns that may be passed in kwargs
        extra_cols = [
            'position_id', 'strategy_id', 'requested_pct', 'requested_sol_amount', 'requested_token_amount',
            'executed_sol_amount', 'executed_token_amount', 'price_usd', 'price_sol', 'slippage_bps', 'price_impact_pct',
            'quote_json', 'route_plan_json', 'jito_tip_lamports', 'priority_fee_lamports', 'tx_signature', 'bundle_id',
            'error_code', 'error_message', 'provider', 'latency_ms'
        ]
        ...
        try:
            ...
            if row:
                return dict(row)
            # If writing system event fails, don't recurse infinitely
            logger.exception("append_trade_event integrity failed", extra={"idempotency_key": idempotency_key})
            raise
        except Exception as e:
            logger.exception("append_trade_event failed", extra={"error": str(e), "idempotency_key": idempotency_key})
            raise
    ...
    async def insert_bandit_observation(
        ...
        try:
            ...
            await self.db.execute(
                ...
                (token_mint, position_id, strategy_id, 1 if is_live else 0, action_json, feature_vector_json, created_at, discovery_event_id),
            )
            await self.db.commit()
        except Exception as e:
            await self.append_system_event("ERROR", "DB", "insert_bandit_observation failed", str({"error": str(e), "token": token_mint}))
            raise

    async def finalize_bandit_observation(self, observation_id: int, reward_json: str, final_net_pnl_pct: float, exit_reason: str):
        finalized_at = datetime.now(timezone.utc).isoformat()
        try:
            await self.db.execute("BEGIN")
            await self.db.execute("UPDATE bandit_observations SET reward_json = ?, final_net_pnl_pct = ?, exit_reason = ?, finalized_at = ? WHERE id = ?", (reward_json, final_net_pnl_pct, exit_reason, finalized_at, observation_id))
            await self.db.commit()
        except Exception as e:
            await self.append_system_event("ERROR", "DB", "finalize_bandit_observation failed", str({"error": str(e), "id": observation_id}))
            raise

    async def list_token_bandit_observations(self, token_mint: str, limit: int = 100) -> List[Dict[str, Any]]:
        async with self.db.execute("SELECT * FROM bandit_observations WHERE token_mint = ? ORDER BY id DESC LIMIT ?", (token_mint, limit)) as cur:
            rows = await cur.fetchall()
        return [dict(r) for r in rows]
