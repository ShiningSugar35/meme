...
async def _ensure_live_group(repo, name="test_live"):
    ...
    if not live:
        await repo.create_strategy_group(name, 0.2, is_live=True, priority=10, raw_config_json='{}')
        groups = await repo.list_strategy_groups()
    return [g for g in groups if g['is_live']]


@pytest.mark.asyncio
async def test_only_one_live_position_per_token(repo, pipeline_factory, monkeypatch):
    ...
    buy_submitted = [t for t in tes if t['token_mint'] == token_mint and t['event_type'] == 'BUY_PENDING' and t['is_live'] == 1]
...
async def test_simulated_positions_for_losers(repo, pipeline_factory):
    ...
    if not sim_groups:
        await repo.create_strategy_group("test_sim", 0.2, is_live=False, priority=100, raw_config_json='{}')
        groups = await repo.list_strategy_groups()
        sim_groups = [g for g in groups if not g['is_live']]
    token_mint = 'PASS1'
    ...
    for p in positions:
        ...
        assert p.get('status') in ['POSITION_OPEN', 'SIM_OPEN', 'CLOSED']


@pytest.mark.asyncio
async def test_jupiter_high_impact_blocks(repo, pipeline_factory, monkeypatch):
    monkeypatch.setattr(type(settings), "get_provider_mode", lambda self: ProviderMode.MOCK)
...
@pytest.mark.asyncio
async def test_duplicate_token_no_second_live(repo, pipeline_factory, monkeypatch):
    monkeypatch.setattr(type(settings), "get_provider_mode", lambda self: ProviderMode.MOCK)
    pipeline, mock = pipeline_factory()
    pipeline.jito.send = AsyncMock(return_value={"ok": True, "signature": "mock_sig", "bundle_id": "mock_bundle"})
    live_groups = await _ensure_live_group(repo, "test_dup")
    discovery_event_id, _ = await repo.create_discovery_event_idempotent(token_mint='PASS1')
    await pipeline.handle_token_second_filter_result('PASS1', live_groups, None, discovery_event_id=discovery_event_id)
