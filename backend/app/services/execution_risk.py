...
EXECUTION_RISK_VERSION = "severe_stop_gap_entry_pit_v1"
...
class ExecutionRiskModel:
    ...
    def predict_probability(self, row: Mapping[str, Any]) -> float:
        ...
        probability = float(self.estimator.predict_proba(self._vector(row))[0, 1])
        if not np.isfinite(probability):
            raise RuntimeError("execution-risk model returned a non-finite probability")
...
class ExecutionRiskTrainer:
    ...
    def _rows(self) -> list[dict[str, Any]]:
        return self.database.fetch_all(
            """
            SELECT p.id AS position_id,p.sample_id,p.stop_loss_price,p.metadata_json,
            ...
            WHERE p.account_kind='simulation'
              ...
              AND p.status='closed'
              AND p.exit_reason LIKE 'stop_loss%'
    ...
    def train(self) -> ExecutionRiskTrainingResult:
        raw_rows = self._rows()
        ...
        # showed a strong age-dependent stop-gap mechanism.
        ...
        usable = [
            name for name, values in materialized.items()
            if sum(value is not None for value in values) >= int(0.70 * observations)
        ]
        if len(usable) < 3:
            return ExecutionRiskTrainingResult(
                available=False,
        ...
        for name, factory in factories.items():
            ...
            metrics["certified"] = bool(
                metrics["auc"] >= MIN_AUC
                and metrics["ap"] >= holdout_prevalence + MIN_AP_LIFT
            )
            candidates.append((name, estimator, metrics))
        certified = [item for item in candidates if item[2]["certified"]]
        public_candidates = tuple({"algorithm": name, **metrics} for name, _, metrics in candidates)
        if not certified:
            return ExecutionRiskTrainingResult(
                available=True,
                certified=False,
                ...
                negatives=negatives,
                prevalence=prevalence,
                reason="certification_metrics_failed",
                candidates=public_candidates,
            )
        name, estimator, metrics = max(certified, key=lambda item: (item[2]["ap"], item[2]["auc"]))
        digest = hashlib.sha256()
        for (row, target) in labelled[:split]:
        ...
        model = ExecutionRiskModel(
            ...
            metrics={
                **metrics,
                ...
                "prevalence": prevalence,
                "train_rows": split,
                "target": "trigger_reference_price <= stop_loss_price * 0.90",
                "pit_only": True,
            },
        )
        return ExecutionRiskTrainingResult(
