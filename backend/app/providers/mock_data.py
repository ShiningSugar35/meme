...
class MockData:
    def __init__(self):
        ...
        self.tokens = {
            'PASS1':     dict(token_mint='PASS1', **self._base_token_def),
            'PASS1_150': dict(token_mint='PASS1_150', **self._base_token_def),
            'PASS1_510': dict(token_mint='PASS1_510', **self._base_token_def),
            'FAIL_INIT': {
        ...
        self.latest = {
            ...
            'PASS1_150': {'price': 1.5, 'calls': 0, 'price_usd': 1.5, 'sol_price': 1.5, 'sol_liquidity': 1000},
            'PASS1_510': {'price': 1.5, 'calls': 0, 'price_usd': 1.5, 'sol_price': 1.5, 'sol_liquidity': 1000},
    ...
    def _refresh_all(self):
        now = datetime.now(timezone.utc)
        self._last_refresh = now
        pool_ages = {'PASS1': 150, 'PASS1_150': 150, 'PASS1_510': 510, 'FAIL_INIT': 150, 'FAIL_SECOND': 150}
        for mint, t in self.tokens.items():
            age = pool_ages.get(mint, 150)
            t['pool_created_at'] = (now - timedelta(seconds=age)).isoformat()
        self.klines = {
            'PASS1': [
            ...
            'PASS1_510': [
                {'open_time': (now - timedelta(minutes=9)).isoformat(), 'close': 1.0},
                {'open_time': (now - timedelta(minutes=6)).isoformat(), 'close': 1.5},
                {'open_time': (now - timedelta(minutes=3)).isoformat(), 'close': 2.0},
            ],
            'FAIL_SECOND': [
                {'open_time': (now - timedelta(minutes=5)).isoformat(), 'close': 1.0},
